...
export const AppConfirmationModal: React.FC<AppConfirmationModalProps> = ({
  ...
  return (
      <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
          ...
          <div className={`bg-slate-900 border ${isDangerMode ? 'border-red-500/50' : 'border-white/10'} rounded-2xl p-6 max-w-sm w-full shadow-2xl scale-100 animate-in zoom-in-95 duration-200 ${isShaking ? 'shake' : ''}`}>
              ...
              <div className="flex gap-3">
                  ...
                  <button 
                      ...
                      `}
                      disabled={isDangerMode && !isValid}
                  >
                      {isDangerMode ? 'Wipe Everything' : 'Yes, Delete'}
                  </button>
              </div>
          </div>
      </div>
