...
export const SocialPanel: React.FC = () => {
    ...
    const handleAddFriend = async () => {
        ...
        setRequestSent(true);
        window.dispatchEvent(new CustomEvent('rocky-speak', { detail: { text: "Friend request sent!", state: "Happy" } }));
    };

    const handleAcceptFriend = async (uid: string) => {
        await dbService.acceptFriendRequest(uid);
        window.dispatchEvent(new CustomEvent('rocky-speak', { detail: { text: "Friend request accepted!", state: "Happy" } }));
    };
