...
export const ThemeProvider: React.FC<{children: React.ReactNode}> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    if (saved && typeof saved === 'string' && !saved.startsWith('{')) {
        setAccent(saved as AccentColor);
    } else if (saved) {
        // If it's an object or invalid, reset it
        localStorage.removeItem('ekagrazone_theme_accent');
        setAccent('cyan');
    }
    ...
    const handleStorageUpdate = () => {
        ...
        if (updated && typeof updated === 'string' && !updated.startsWith('{')) {
            setAccent(updated as AccentColor);
        } else {
            // Reset to default if key removed (factory reset) or invalid
            setAccent('cyan');
        }
    };
  ...
  const updateAccent = (newAccent: AccentColor) => {
    if (typeof newAccent !== 'string') {
        console.warn('Attempted to set non-string accent color:', newAccent);
        return;
    }
    setAccent(newAccent);
