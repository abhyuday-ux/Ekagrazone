...
import { 
    LogOut, LogIn, Palette, Settings, Target, Clock, Zap, 
    ToggleRight, ToggleLeft, ImageIcon, BookOpen, ChevronRight, 
    Database, RefreshCw, HardDrive, Download, AlertCircle, Trash2, 
...
import { ACCENT_COLORS, WALLPAPERS } from '../types';

export const SettingsPage: React.FC = () => {
    const { 
        targetHours, setTargetHours, dayStartHour, setDayStartHour,
        enableZenMode, setEnableZenMode, wallpaper, setWallpaper,
        showWallpaperOnHome, setShowWallpaperOnHome,
        setIsSubjectManagerOpen, isSyncing, handleSync,
        requestClearToday, requestClearAll, handleExport
    } = useGlobalContext();
    
    const { accent, setAccent } = useTheme();
    const { currentUser, isGuest, logout, signInWithGoogle } = useAuth();
    const { graphicsQuality, setGraphicsQuality } = usePerformance();
    ...
    return (
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-5xl mx-auto w-full pb-24">
            {/* Profile Card */}
